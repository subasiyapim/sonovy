<template>
    <button :disabled="disabled" class="radius-8 px-3 bg-white  " :class="getButtonStyle">
     <div class="flex gap-2 items-center justify-center">
        ...
        <div v-else class="flex items-center gap-1">
            <slot name="icon"></slot>
            <p class="label-sm" :class="getButtonlabelStyle">
                <slot />
            </p>
        </div>
...
import {computed} from 'vue';
    ...
    const getButtonStyle = computed(() => {
        ...
        if(props.type == 'error'){
            buttonClass += ' border border-error-500 '
        }else {
            if(props.disabled) {
...
const getButtonlabelStyle = computed(() => {
        let buttonClass = '';


        if(props.type == 'error'){
            buttonClass += ' c-error-500 '
        }else {
            if(props.disabled){
                buttonClass += ' c-soft-300  '
            }else {
                 buttonClass += ' c-sub-600 '
            }
