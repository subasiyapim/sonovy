<template>
 ...
 <tippy  :maxWidth="440" ref="myTippy" theme="light" :allowHtml="true" :sticky="true" :trigger="confirmDelete ? 'click' : 'manual'"
            :interactive="true" :appendTo="getBody">
        <button :type="type" class="radius-full rounded-full  flex items-center justify-center" :class="getButtonStyle">
        ...
        <template #content>
            <ConfirmDeleteDialog @confirm="onConfirm" @cancel="onCancel"
                                    :title="title"
...
import {ConfirmDeleteDialog} from '@/Components/Dialog'
    ...
    const getButtonStyle = computed(() => {
        ...
        if(props.size == 'medium'){
         ...
         if(props.size == 'large'){
            buttonClass += ' h-16 w-16 '
        }
        if(props.hasBorder != null || props.hasBorder == true){
           buttonClass += ' border border-soft-200 ';
        }
        return buttonClass;
    })
