    ...
    <div class="flex items-center mb-4">
     <div class="flex-1 flex items-center gap-7">
        ...
        <div>
            ...
            <select class="smallSelect mt-1 inline !appearance-none  iconless paragraph-xs !font-semibold border-none focus:outline-none focus:ring-0 c-blue-500">
       ...
       <div class="flex items-center gap-2">
            <AppTextInput  placeholder="Ara...">
                <template #icon><SearchIcon color="var(--sub-600)"/></template>
            </AppTextInput>
  ...
  <template v-if="!(data == null || data.length <= 0)">
    ...
    <p class="w-28">
     ...
     <div class="flex flex-1 justify-center  gap-3">
        ...
        <a :disabled="!tableData.first_page_url" :href="tableData.first_page_url" class="p-2 radius-8 w-10 h-10 flex items-center justify-center">
            <ArrowDoubleLeftIcon color="var(--sub-600)" />

        </a>
        <a :disabled="!tableData.prev_page_url"  :href="tableData.prev_page_url"  class="p-2 radius-8 w-10 h-10 flex items-center justify-center">
            <ChevronLeftIcon color="var(--sub-600)" />

        </a>


        <Link :href="route('control.artists.index',{page:p})" v-for="p in Math.ceil((tableData.total / tableData.per_page)) >= 7 ? 6 : Math.ceil((tableData.total / tableData.per_page))" class="p-2 radius-8 w-10 h-10 border border-soft-200-500 flex items-center justify-center">
            {{p}}
        </Link>


        <a :disabled="!tableData.next_page_url" :href="tableData.next_page_url" class="p-2 radius-8 w-10 h-10 flex items-center justify-center">
             <ChevronRightIcon color="var(--sub-600)" />

        </a>
        <a :disabled="!tableData.last_page_url" :href="tableData.last_page_url" class="p-2 radius-8 w-10 h-10 flex items-center justify-center">
      ...
      <div class="w-28 max-w-xs mx-auto">
            ...
            <select id="options" name="options" class="mt-1 block w-full pl-3 pr-10 py-2 paragraph-sm border border-soft-200 focus:outline-none  radius-8">
...
<script setup >
    ...
    import {TableOrderIcon,SearchIcon,ChevronLeftIcon,ChevronRightIcon,ArrowDoubleLeftIcon,ArrowDoubleRightIcon} from '@/Components/Icons';
    ...
    const render = (e) => {
        ...
        return h(slots.default()[e.colIndex], {row:data.value[e.rowIndex], index:e.rowIndex},  slots.default()[e.colIndex].children);
    };
