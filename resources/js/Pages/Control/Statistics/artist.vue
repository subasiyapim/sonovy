    ...
    <div class="bg-white-400 h-44 p-6 relative mb-24">
      ...
      <div class="flex items-center justify-end gap-2 absolute top-5 right-5 w-[600px] flex-wrap">
        ...
        <div v-for="(platform,index) in artist.download_platforms" class="flex items-center gap-1">
          ...
          <template v-if="index < artist.download_platforms?.length - 1"><span class="label-sm c-soft-400 mx-2">•</span>
          </template>
        </div>
      </div>
    </div>
      ...
      <div class="grid grid-cols-2 gap-3 mb-5">
        <AppCard class="flex-1 w-full">
          ...
          <template #body>
            ...
            <div
                v-if="platformStatistics  && (platformStatistics?.spotify > 0 || platformStatistics?.apple > 0 || platformStatistics?.other > 0)"
                class="flex flex-col gap-4">
              <div>
                <PlatformsTotalStreamChart
                    :data="{ platforms: platformStatistics || { spotify: 0, apple: 0, other: 0 } }"/>
              </div>
              <hr>
        ...
        <PlatformBasedSalesCountChart :platform-sales-count="platformSalesCount || {}" :artist_id="props.artist.id"
                                      :platforms="platforms"/>
