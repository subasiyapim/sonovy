...
class Broadcast extends Model implements HasMedia
{
    ...
    {
        $this->addMediaCollection('broadcasts')
            ->singleFile()
    ...
    {
        $file = $this->getMedia('broadcasts')->last();
        if ($file) {
    ...
    protected function statusText(): Attribute
    {
        return Attribute::make(
            get: fn() => BroadcastStatusEnum::getTitles()[$this->status]
        );
    }
    ...
    protected function statusClass(): Attribute
    {
        return Attribute::make(
            get: function () {
                $styles = [
                    BroadcastStatusEnum::NEW->value => 'default-badge',
                    BroadcastStatusEnum::WAITING_FOR_APPROVAL->value => 'dark-badge',
                    BroadcastStatusEnum::APPROVED->value => 'green-badge',
                    BroadcastStatusEnum::REJECTED->value => 'red-badge',
                    BroadcastStatusEnum::NOT_BROADCASTING->value => 'yellow-badge',
                    BroadcastStatusEnum::DRAFT->value => 'yellow-badge',
                ];
    ...
    {
        return Attribute::make(
            get: fn() => BroadcastPublishedCountryTypeEnum::getTitles()[$this->publish_country_type],
        );
    }
    ...
    {
        return $this->belongsToMany(Artist::class, 'artist_broadcast', 'broadcast_id', 'artist_id')->withPivot('is_main');
    }
    ...
    {
        return $this->belongsToMany(Country::class, 'broadcast_published_country', 'broadcast_id', 'country_id');
    }

    public function downloadPlatforms(): BelongsToMany
    {
        return $this->belongsToMany(Platform::class, 'broadcast_download_platform', 'broadcast_id', 'platform_id')
            ->withPivot('price', 'pre_order_date', 'publish_date');
    }

    public function songs(): BelongsToMany
    {
        return $this->belongsToMany(Song::class, 'broadcast_song', 'broadcast_id', 'song_id');
    }
